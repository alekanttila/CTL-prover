...
public class ConcurrentBreadthTableau extends ConcurrentTableau {


    public ConcurrentBreadthTableau(Formula f) {
        super(f);
        multiUpLinks = false;
    ...
    protected ExtendResult checkLevel(int level, boolean checkAll, ConcurrentUpLinkTree<Pair<Colour, Hue>> checkedUpLinks) {
        ...
        if (!checkAll) {
            if (result == SUCCESS) {
                ...
                statusPrint(TABLEAU, SOME, "LGRUNS " + lgRuns.toString() + " tableaus " + tableausBuilt.toString());
            }
        } else {
    ...
    class ColourCheckTask extends RecursiveTask<Node> {
        protected ConcurrentBreadthTableau t;
        protected ConcurrentUpLinkTree<Pair<Colour, Hue>> checkedUpLinks;
        ...
        private ColourCheckTask(Colour c, int level, boolean checkAll, ConcurrentUpLinkTree<Pair<Colour, Hue>> checkedUpLinks, int taskID) {
        ...
        protected Node compute() {
            ...
            for (int i = 0; i < n.z.size(); i++) {
                ...
                ConcurrentUpLinkTree<Pair<Colour, Hue>> hueUpLinks = checkedUpLinks.getUpLinks(new Pair (c, c.first()), hueToCheck);
                if (hueUpLinks != null) {
...
private ExtendResult checkLevel(ColourSet possibleColours, HueSet possibleHues, Node parent, Hue parentHue, int level, boolean checkAll, ConcurrentUpLinkTree<Pair<Colour, Hue>> checkedUpLinks, AtomicInteger lgRuns, AtomicInteger tableausBuilt, int taskID) {
        ExtendResult result = FAILURE;
        sectionPrint(TABLEAU, SOME,"Task " + taskID +  "LEVEL " + level);
        COLOUR_LOOP:
        for (Colour c : possibleColours) {
            ...
            for (Hue firstHue : possibleHues) {
                statusPrint(TABLEAU, MAX,"Task " + taskID +  "Checking first hue " + firstHue.name);

                if (c.contains(firstHue)) {
                    statusPrint(TABLEAU, SOME, "Task " + taskID + "Fixing first hue " + firstHue.name);
                    ...
                    for (int i = 0; i < n.z.size(); i++) {
                        ...
                        //CHECK FOR ALREADY CHECKED UPLINKS HERE
                        ConcurrentUpLinkTree<Pair<Colour, Hue>> hueUpLinks = checkedUpLinks.getUpLinks(new Pair(c, firstHue), hueToCheck);
                        if (hueUpLinks != null) {
                            NodeHue checkedChild = hueUpLinks.getNodeHue();
